...
function Send({ togglePopup, coinRanges, userData, company_id }) {
  ...
  const handleInputChange = (e) => {
    ...
    //   (r) =>
    //     Number(validValue) >= r.min_coins && Number(validValue) <= r.max_coins
    // );
    const range = coinRanges.reduce((max, item) => {
      if (
        Number(validValue) < item.min_coins ||
        Number(validValue) > item.max_coins
      ) {
        return max;
      }
      return max === null || item.rate > max.rate ? item : max;
    }, null);
  ...
  return (
    ...
    >
      <div
        className="bg-[#1B1A1A] p-4 sm:p-6 rounded-t-3xl shadow-xl max-w-lg relative"
      ...
      >
        ...
        <p className="text-sm sm:text-base text-[#B0B0B0] text-center mb-6">
          Please enter the amount and your UPI ID to generate the QR code for
          selling your coins.
        </p>
