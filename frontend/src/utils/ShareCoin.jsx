...
function ShareCoin({
  ...
  return (
    <div
      className="fixed inset-x-0 top-0 flex justify-center bg-transparent bg-opacity-40 backdrop-blur-sm z-50"
    ...
    >
      ...
      >
        ...
        <p className="text-sm sm:text-base text-[#B0B0B0] text-center mb-6">
          Invite your friends to join and experience the benefits of our coin
          system together!
        </p>
        ...
        <input
          ...
          className="w-full uppercase p-2 sm:p-3 bg-[#2C2C2C] text-white border border-transparent rounded-lg mb-1 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#505050] transition duration-300 text-sm sm:text-base"
        />
        <p
          className={`h-5 mb-1 flex items-center ${
            apiError ? "text-red-700" : "text-white"
          }`}
        >
          {sendData.receiver ? `Receiver : ${sendData.receiver}` : apiError}
        </p>
        <input
